...
// UsersLog will log all the user edit logs inside a file
var UsersLog = source.LOGGER.NewFileLogger(filepath.Join(source.ASSETS, "logs", "userEditLogs.log"), int64(source.OPTIONS.Ints("branding", "recycle_log")))

// Users has multiple subcommands
var Users = commands.ROOT.NewCommand(&commands.Command{
	Aliases:     []string{"users", "clients"},
	Permissions: []string{interactions.ADMIN, interactions.MOD, interactions.RESELLER},
	Description: "moderate and manage users",
		...
		for _, user := range users {
			...
			if len(sessions.IndexSessions(user.Username)) >= 1 {
				tablet.Rows[len(tablet.Rows)-1].Columns[1] = &gotable2.Column{
					Text:  session.ExecuteBrandingToStringNoErr(map[string]any{"row": user.User()}, "commands", "users", "value_username_online.tfx"),
					Align: gotable2.AlignLeft,
				}
			}
			...
			if user.API {
				tablet.Rows[len(tablet.Rows)-1].Columns[1] = &gotable2.Column{
					Text:  session.ExecuteBrandingToStringNoErr(map[string]any{"row": user.User()}, "commands", "users", "value_username_api.tfx"),
					Align: gotable2.AlignLeft,
				}
			}
			...
			if database.DB.IsSuperuser(user) {
				tablet.Rows[len(tablet.Rows)-1].Columns[1] = &gotable2.Column{
					Text:  session.ExecuteBrandingToStringNoErr(map[string]any{"row": user.User()}, "commands", "users", "value_username_superuser.tfx"),
					Align: gotable2.AlignLeft,
				}
			}
