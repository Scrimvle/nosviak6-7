...
var UsersSessions = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"max_sessions"},
	Description: "max number of sessions modifier",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if functions.CanAccessThemPermissions(s.User, "!admin", "!mod", "reseller") && s.User.Sessions < item {
		...
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to set %s's sessions limiter: %v", user.Username, err)
		...
		sessions.WriteToSession(sessions.IndexSessions(user.Username), func(index *sessions.Session) {
			...
			index.Reader.PostAlert(&terminal.Alert{
				AlertMessage: index.ExecuteBrandingToStringNoErr(map[string]any{"promoter": s.User.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.Sessions}, "commands", "users", "alerts", "tunable_changed.tfx"),
				AlertCode:    terminal.MESSAGE,
			})
		})

		reg.UsersLog.WriteLog("%s has adjusted %s max sessions to %d", s.User.Username, user.Username, user.Sessions)
		return s.ExecuteBranding(map[string]any{"target": user.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.Sessions}, "commands", "users", "alerts", "performer_tunable_set.tfx")
	},
