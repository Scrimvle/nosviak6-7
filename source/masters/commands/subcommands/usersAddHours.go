...
var UsersAddHours = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"add_hours", "add_hrs"},
	Description: "add hours to a users expiry",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		user.Expiry += int64((item * 60) * 60)
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to set %s's expiry: %v", user.Username, err)
