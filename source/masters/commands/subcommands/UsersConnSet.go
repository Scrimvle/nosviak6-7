...
var UsersConns = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"conns", "concurrents"},
	Description: "change the concurrents of a user",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if functions.CanAccessThemPermissions(s.User, "!admin", "!mod", "reseller") && s.User.Conns < item {
		...
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to set %s's conns: %v", user.Username, err)
