...
var UsersCooldown = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"cooldown"},
	Description: "change the cooldown of a user",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to set %s's cooldown: %v", user.Username, err)
