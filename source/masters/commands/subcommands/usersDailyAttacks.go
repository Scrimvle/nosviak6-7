...
var UsersDailyAttacks = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"daily_attacks"},
	Description: "change the max daily attacks from a user",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if functions.CanAccessThemPermissions(s.User, "!admin", "!mod", "reseller") && s.User.MaxAttacks < item {
			return s.ExecuteBranding(map[string]any{"target": user.User(), "value": item, "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", "")}, "commands", "users", "above_reseller_tunable.tfx")
		}

		user.MaxAttacks = item
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to set %s's max attacks: %v", user.Username, err)
		...
		sessions.WriteToSession(sessions.IndexSessions(user.Username), func(index *sessions.Session) {
			...
			index.Reader.PostAlert(&terminal.Alert{
				AlertMessage: index.ExecuteBrandingToStringNoErr(map[string]any{"promoter": s.User.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.MaxAttacks}, "commands", "users", "alerts", "tunable_changed.tfx"),
				AlertCode:    terminal.MESSAGE,
			})
		})

		reg.UsersLog.WriteLog("%s has changed %s's max attacks to %d", s.User.Username, user.Username, user.Maxtime)
		return s.ExecuteBranding(map[string]any{"target": user.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.MaxAttacks}, "commands", "users", "alerts", "performer_tunable_set.tfx")
	},
