...
var PlansTokensCreate = PlansTokens.NewCommand(&commands.Command{
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		duration, err := strconv.Atoi(ac.Args[1].Values[0].ToString())
		if err != nil {
		...
		token := &database.Token{
			...
			Plan: ac.Args[0].Values[0].ToString(),
		...
		return s.ExecuteBranding(map[string]any{"token": token.Token, "plan": token.Plan}, "commands", "plans", "tokens_created.tfx")
	},
