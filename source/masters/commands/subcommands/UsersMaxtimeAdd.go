...
var UsersAddMaxtime = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"add_maxtime", "add_time"},
	Description: "add onto the maxtime of a user",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if functions.CanAccessThemPermissions(s.User, "!admin", "!mod", "reseller") && s.User.Maxtime < item {
			return s.ExecuteBranding(map[string]any{"target": user.User(), "value": user.Maxtime + item, "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", "")}, "commands", "users", "above_reseller_tunable.tfx")
		}

		user.Maxtime += item
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to change %s's maxtime: %v", user.Username, err)
		...
		sessions.WriteToSession(sessions.IndexSessions(user.Username), func(index *sessions.Session) {
			...
			index.Reader.PostAlert(&terminal.Alert{
				AlertMessage: index.ExecuteBrandingToStringNoErr(map[string]any{"promoter": s.User.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.Maxtime}, "commands", "users", "alerts", "tunable_changed.tfx"),
				AlertCode:    terminal.MESSAGE,
			})
		})

		reg.UsersLog.WriteLog("%s has changed %s's maxtime to %d", s.User.Username, user.Username, user.Maxtime)		
		return s.ExecuteBranding(map[string]any{"target": user.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.Maxtime}, "commands", "users", "alerts", "performer_tunable_set.tfx")
	},
