...
var UsersAddCooldown = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"add_cooldown"},
	Description: "add onto the cooldown of a user",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if functions.CanAccessThemPermissions(s.User, "!admin", "!mod", "reseller") && s.User.Cooldown > item {
			return s.ExecuteBranding(map[string]any{"target": user.User(), "value": user.Cooldown + item, "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", "")}, "commands", "users", "above_reseller_tunable.tfx")
		}

		user.Cooldown += item
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to change %s's cooldown: %v", user.Username, err)
		...
		sessions.WriteToSession(sessions.IndexSessions(user.Username), func(index *sessions.Session) {
			...
			index.Reader.PostAlert(&terminal.Alert{
				AlertMessage: index.ExecuteBrandingToStringNoErr(map[string]any{"promoter": s.User.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.Cooldown}, "commands", "users", "alerts", "tunable_changed.tfx"),
				AlertCode:    terminal.MESSAGE,
			})
		})

		reg.UsersLog.WriteLog("%s has changed %s's cooldown to %d", s.User.Username, user.Username, user.Cooldown)
		return s.ExecuteBranding(map[string]any{"target": user.User(), "field": strings.ReplaceAll(strings.ReplaceAll(ac.Command.Aliases[0], "add_", ""), "set_", ""), "value": user.Cooldown}, "commands", "users", "alerts", "performer_tunable_set.tfx")
	},
