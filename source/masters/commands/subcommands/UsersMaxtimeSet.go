...
var UsersMaxtime = reg.Users.NewCommand(&commands.Command{
	Aliases:     []string{"maxtime", "time"},
	Description: "change the maxtime of a user",
	...
	CommandFunc: func(ac *commands.ArgContext, s *sessions.Session) error {
		...
		if err := database.DB.EditUser(user, s.User, commands.Conn.SendWebhook); err != nil {
			source.LOGGER.AggregateTerminal().WriteLog(gologr.ERROR, "Error while trying to set %s's maxtime: %v", user.Username, err)
