...
func (memory *Memory) args(args []*swash.Token) (*swash.Token, error) {
	if len(args) == 0 {
		return swash.UNDEFINED, nil
	}

	constant, err := memory.evalToken(args[0])
	if err != nil {
		return nil, err
	}

	for p := 2; p < len(args); p += 2 {
		arg, err := memory.evalToken(args[p])
		if err != nil || arg == nil {
...
func (memory *Memory) evalToken(token *swash.Token) (*swash.Token, error) {
	switch token.TokenType {

	case swash.INDENTWORK:
		return memory.args(token.TokenArgs)

	case swash.NUMBER, swash.STRING, swash.BOOLEAN:
		return token, nil

	case swash.VARIABLE, swash.FUNCTION:
		value, err := memory.index(token)
