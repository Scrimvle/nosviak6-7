...
/*
	...
	over all the nodes within the source code and attempt to generate responses to them all,
	but it doesn't have any sort of order of operations as we don't want this to be implemented.
*/

// VERSION is the version of this swash version
...
func NewEvaluator(tokenizer *swash.Tokenizer, wr io.Writer) *Evaluator {
	eval := &Evaluator{
		tokenizer: tokenizer,
		standard:  new(standard),
		Memory:    NewMemory(wr, make(map[string]any)),
	}
...
// Execute is the main keyword which will execute the source code
...
func (evaluator *Evaluator) self(objects []swash.ParserNode, leaves *swash.TokenType, forced bool) (*swash.Token, bool, error) {
	for i, object := range objects {
		...
		case *swash.Token:
			...
			case swash.TEXT_BLANKLINE:
				if i+1 >= len(objects) {
					continue
				}

				evaluator.Memory.wr.Write([]byte("\r\n"))

			case swash.TEXT: // evaluate the expression
				payload := bytes.NewBuffer(make([]byte, 0))
				payload.WriteString(index.TokenLiteral)

				if len(index.TokenArgs) > 0 && index.TokenArgs[0].TokenType == swash.TEXT_BLANKLINE && !strings.HasSuffix(payload.String(), "\r\n") {
					payload.Write([]byte("\r\n"))
				}
