...
func NewMemory(wr io.Writer, pkgs map[string]any) *Memory {
	memory := &Memory{
		packages: pkgs,
		memory:   make([]any, 0),
		wr:       wr,
...
func (memory *Memory) WritePackage(pkg string, v any) error {
	if _, ok := memory.packages[pkg]; ok {
		return errors.New("package already imported")
	}

	memory.packages[pkg] = v
	return nil
}
...
func (memory *Memory) index(token *swash.Token) (*swash.Token, error) {
	allocatedMemory := NewMemory(memory.wr, memory.packages)
...
func (memory *Memory) execute(f *Function, t *swash.Token) (*swash.Token, error) {
	evaluator := NewEvaluator(f.Tokenizer, memory.wr)
	...
	/* replaces all the different variables within the source */
