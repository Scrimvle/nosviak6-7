...
var TIMEFunctions map[string]any = map[string]any{
...
func Until(unix int) string {
	return humanize.Time(time.Unix(int64(unix), 0))
}

func Since(unix int) string {
	return humanize.RelTime(time.Unix(int64(unix), 0), time.Now(), "ago", "from now")
}