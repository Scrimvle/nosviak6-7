...
func (tokenizer *Tokenizer) ApplyNodes(nodes []ParserNode) {
	tokenizer.tokenizerParserNodes = nodes
}
...
func (tokenizer *Tokenizer) PeekNext() (*Token, bool) {
	...
	token := tokenizer.tokenize(strings.Split(tokenizer.tokenizerTarget[tokenizer.tokenizerLine], "")[0])
	if token == nil {
		return nil, true
	}
...
// Line will return the current line and it's content
func (tokenizer *Tokenizer) Line() string {
	return tokenizer.tokenizerTarget[tokenizer.tokenizerLine]
}

// tokenize will take the current x value and attempt to form it into a single token
func (tokenizer *Tokenizer) tokenize(x string) *Token {
	...
	case '"', '\'', '`': // String operator
		tokenizer.shift(1)
		inspector := tokenizer.runUntilVoid(0, singleCharTyper(rune(x[0])))
		tokenizer.shift(1)
	...
	default: // Indent, int & space operators
		if unicode.IsLetter(rune(x[0])) && len(x) == 1 {
